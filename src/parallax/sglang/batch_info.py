...
def _build_no_prefix_tree_cache(model_runner: ModelRunner):
    """Build sglang-native no-prefix cache for allocation/scheduling.

    For hybrid SWA models (e.g. Step3.5), use SWAChunkCache so schedule_batch
    can run SWA-specific eviction logic. For others, use ChunkCache.
    """
...
def form_sgl_batch_prefill(
    ...
    tree_cache = page_tree_cache if page_tree_cache is not None else _build_no_prefix_tree_cache(
        model_runner
    )
