...
def request_to_proto(
    ...
    IntermediateRequest contains request_id, current_position, status, hidden_states,
    and optional residual states for PP-compatible backends.
    """
    ...
    for request in requests:
        ...
        if hasattr(request, "residual") and request.residual is not None:
            proto_req.residual = tensor_to_bytes(request.residual, device=device)
...
def proto_to_request(
    ...
    for proto_req in proto_request.reqs:
        ...
        residual = None
        if hasattr(proto_req, "residual") and proto_req.residual:
            residual = bytes_to_tensor(proto_req.residual, device)
