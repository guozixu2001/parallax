...
class Request:
    ...
    def __init__(
        ...
        # Speculative decoding: number of draft tokens to generate
    ...
    def is_speculative(self) -> bool:
        """Checks if the request is in the speculative decoding stage."""
        return self.status == RequestStatus.SPECULATIVE
...
class InitialRequest(Request):
    ...
    def commit_new_token(self, token_id: int, enable_speculative: bool = False):
        """
        ...
        Args:
            token_id: The new token ID to commit
            enable_speculative: If True, transition to SPECULATIVE instead of DECODING
        """
        if self.is_finished:
            logger.warning(
        ...
        if self.status == RequestStatus.PREFILLING:
            if enable_speculative:
                self.status = RequestStatus.SPECULATIVE
            else:
...
class IntermediateRequest(Request):
    ...
    def __init__(
        ...
        self.return_probs = return_probs
        # Speculative decoding fields
        # draft_token_ids: tokens generated by draft model (First Peer → Last Peer)
        # num_accepted_tokens: number of tokens accepted during verification (Last Peer → First Peer)
    ...
    def from_initial_request(
        ...
        Args:
            ...
            draft_token_ids: Draft tokens for speculative decoding (First Peer → Last Peer).
            num_accepted_tokens: Number of accepted draft tokens (Last Peer → First Peer).
    ...
    def from_intermediate_request(
        ...
        Args:
            old_request: The previous intermediate request.
            new_hidden_states: Updated hidden states from the current peer.
            draft_token_ids: Draft tokens for speculative decoding (optional).
            num_accepted_tokens: Number of accepted draft tokens (optional).
        """
        # Preserve speculative fields if not provided
        if draft_token_ids is None:
            draft_token_ids = old_request.draft_token_ids
        if num_accepted_tokens == 0:
            num_accepted_tokens = old_request.num_accepted_tokens
    ...
    def __repr__(self):
        ...
        if self.draft_token_ids is not None:
            fields.append(f"draft_token_ids={self.draft_token_ids}")
        if self.num_accepted_tokens > 0:
            fields.append(f"num_accepted_tokens={self.num_accepted_tokens}")
